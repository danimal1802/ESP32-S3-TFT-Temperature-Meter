// Original by Bodmer and adapted by Daniel Mitchell for specific use 
// 2 April 2024
...
#define BMP_SDA 8 // Define the SDA pin
#define BMP_SCL 9 // Define the SCL pin
...
void setup(void) {
  ...
  Wire.begin(BMP_SDA, BMP_SCL); // Begin I2C communication
  if (!bmp.begin()) { // Initialize BMP280 sensor
...
void analogMeter()
{
  ...
  for (int i = -50; i < 51; i += 5) {
    ...
    if (i % 25 == 0 ) {
      ...
      switch (i / 25) {
        ...
        case 0: tft.drawCentreString("30", x0, y0 - 7, 2); break;
        case 1: tft.drawCentreString("35", x0, y0 - 9, 2); break;
  ...
  tft.drawCentreString("Temp C", M_SIZE*120, M_SIZE*70, 4); // Comment out to avoid font 4
...
void plotNeedle(int value, byte ms_delay)
{
  ...
  if (value < 20) value = 20; // Limit value to emulate needle end stops
  ...
  while (!(value == old_analog)) {
    ...
    float sdeg = map(old_analog, 19, 40, -150, -30); // Map value to angle
    ...
    tft.drawLine(M_SIZE*(120 + 20 * ltx), M_SIZE*(140 - 20), osx, osy, TFT_YELLOW);
    tft.drawLine(M_SIZE*(120 + 20 * ltx + 1), M_SIZE*(140 - 20), osx + 1, osy, TFT_YELLOW);
